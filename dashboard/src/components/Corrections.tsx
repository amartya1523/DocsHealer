...
import { AI_CORRECTIONS, type AICorrection, type CorrectionStatus } from '@/data/mockData';
import { cn, formatDate, confidenceColor } from '@/lib/utils';
...
export function Corrections() {
  const pending = AI_CORRECTIONS.filter(c => c.status === 'pending');
  const rest = AI_CORRECTIONS.filter(c => c.status !== 'pending');
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3 text-xs">
          ...
          <span className="text-green-400">{AI_CORRECTIONS.filter(c => c.status === 'approved').length} approved</span>
          <span className="text-red-400">{AI_CORRECTIONS.filter(c => c.status === 'rejected').length} rejected</span>
        </div>
      </div>

      <div className="grid gap-4">
        {AI_CORRECTIONS.map((corr, i) => {
          ...
          return (
            <motion.div
              ...
              className="bg-[#0f0f17] border border-[#1e1e2e] rounded-xl p-5 hover:border-blue-500/20 transition-all"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                {corr.status === 'pending' && (
                  <div className="flex gap-2">
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-600 hover:bg-green-500 text-white text-xs font-medium transition-all">
                      <Check className="w-3 h-3" /> Approve
                    </button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/15 hover:bg-red-500/25 text-red-400 text-xs font-medium transition-all border border-red-500/20">
                      <X className="w-3 h-3" /> Reject
                    </button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] text-[#94a3b8] text-xs font-medium transition-all border border-[#1e1e2e]">
