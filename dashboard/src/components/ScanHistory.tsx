...
import { useScanHistory } from '@/lib/api/hooks';
...
export function ScanHistory() {
  const { data: scanHistory = [], isLoading, isError, error, refetch } = useScanHistory();
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <QueryState
        ...
        emptyTitle="No scan runs yet"
        emptyDescription="The backend will populate historical scan runs here once workflows have executed."
        skeleton={<TableSkeleton rows={5} cols={10} />}
      >
              ...
              <tbody>
                {scanHistory.map((scan, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="text-xs font-medium text-[#e2e8f0]">{scan.repository.split('/')[1] ?? scan.repository}</div>
                      <div className="text-[10px] text-[#475569]">{scan.repository.split('/')[0] ?? 'backend'}</div>
                    </td>
                    <td className="px-4 py-3">
