...
import { useDocumentation } from '@/lib/api/hooks';
import { CardListSkeleton, QueryState } from '@/components/ui/QueryState';

export function Documentation() {
  const { data, isLoading, isError, error, refetch } = useDocumentation();
  const totalSections = data?.totalSections ?? 0;
  const summaryCards = [
    { label: 'Total Sections', value: totalSections, icon: FileText, color: '#3b82f6' },
    { label: 'Verified', value: data?.verified ?? 0, icon: Shield, color: '#22c55e' },
    { label: 'Needs Review', value: data?.needsReview ?? 0, icon: Clock, color: '#f59e0b' },
    { label: 'Auto Fixed', value: data?.autoFixed ?? 0, icon: Wand2, color: '#8b5cf6' },
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <QueryState
        ...
        error={error}
        onRetry={() => refetch()}
        isEmpty={totalSections === 0 && sections.length === 0}
        emptyTitle="Documentation index is empty"
        emptyDescription="Once the backend indexes repository docs, their sections and health signals will appear here."
      ...
      >
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {summaryCards.map((item, i) => (
            <motion.div
              ...
              className="rounded-[22px] border border-[#1e1e2e] bg-[linear-gradient(180deg,rgba(15,15,23,0.98),rgba(13,20,36,0.92))] p-4"
            >
              ...
              <div className="mt-2 h-1 bg-[#1e1e2e] rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${totalSections > 0 ? (item.value / totalSections) * 100 : 0}%`, background: item.color }}
                />
              </div>
            </motion.div>
          ))}
        ...
        <div className="bg-[#0f0f17] border border-[#1e1e2e] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#1e1e2e] flex items-center justify-between">
            ...
            <span className="text-xs text-[#475569]">Showing {Math.min(sections.length, 6)} of {totalSections}</span>
          </div>
